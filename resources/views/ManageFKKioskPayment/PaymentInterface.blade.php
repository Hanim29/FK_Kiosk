...
<div class="container">
	...
	@if($account_type == "student" || $account_type == "vendor")
		<td>
			@if(isset($payment))
				<a href="{{ route('payments.show', ["paymentID"=>$payment['paymentID']]) }}">
			...
			@foreach ($payments as $payment)
				<tr>
					@if ($payment['paymentStatus'] == "paid")
						<td><a href="{{ route('payments.show', ["paymentID"=>$payment['latestPaymentID']]) }}"><i class="bi bi-eye-fill"></i></a></td>
						<td>{{ $payment['appID'] }}</td>
						<td>
						...
						<td colspan="2">
							<div class="row">
								<a href="{{ route('payments.show', ["paymentID"=>$payment['latestPaymentID']]) }}" class="col-2 btn btn-primary">View</a>
								<form action="{{ route('payments.delete', ['paymentID' => $payment['latestPaymentID']]) }}" method="POST" class="col-2">
									@csrf
									@method('DELETE')
